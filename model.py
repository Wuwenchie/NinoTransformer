...
class Geoformer(nn.Module):
    def __init__(self, mypara):
        ...
        if self.mypara.needtauxy:
            ...
            self.cube_dim = (
                (mypara.input_channal + 3) * mypara.patch_size[0] * mypara.patch_size[1]
            )
        else:
            self.cube_dim = (
    ...
    def encode(self, predictor, in_mask):
        ...
        """
        lb = predictor.size(1)
        # print("Before reshape:", predictor.shape)
        # print("Before reshape:", predictor.shape)
        # print("Expected total:", predictor.size(0)*T*self.cube_dim*(H//self.mypara.patch_size[0])*(W//self.mypara.patch_size[1]))
        # print("Actual total:", predictor.numel())

        predictor = unfold_func(predictor, self.mypara.patch_size)
        # print("after unfold:", predictor.shape)
        # print("cube_dim", self.cube_dim)
    ...
    def decode(self, predictand, en_out, out_mask, enout_mask):
        ...
        T = predictand.size(1)
        # print("Before reshape:", predictand.shape)
        # print("Expected total:", predictand.size(0)*T*self.cube_dim*(H//self.mypara.patch_size[0])*(W//self.mypara.patch_size[1]))
        # print("Actual total:", predictand.numel())
