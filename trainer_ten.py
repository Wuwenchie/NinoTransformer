...
# import math
# import numpy as np
# from model import NinoGeoformer  # 明確導入，避免混淆
# from LoadData import NinoDataset, NinoTestDataset
...
#         self.writer = SummaryWriter(log_dir="./runs/nino_trainer")
...
#                 out_var = self.model(input_var, predict_tar=None, train=False)
...
#                     self.mypara.lon_nino_relative[0]:self.mypara.lon_nino_relative[1]
#                 ].mean(dim=[2, 3])
#                 var_true_list.append(var_true)
#                 nino_true_list.append(nino_true1)
#                 var_pred_list.append(out_var)
#                 nino_pred_list.append(nino_pred1)
#             var_pred = torch.cat(var_pred_list, dim=0)
#             nino_pred = torch.cat(nino_pred_list, dim=0)
#             nino_true = torch.cat(nino_true_list, dim=0)
#             var_true = torch.cat(var_true_list, dim=0)
...
#                 var_pred = self.model(input_var, var_true, train=True, sv_ratio=sv_ratio)
...
#                     self.writer.add_scalar("Train/Loss_Var", loss_var.item(), global_step)
#                     self.writer.add_scalar("Train/Loss_Nino", loss_nino.item(), global_step)
#                     self.writer.add_scalar("Train/Combine_Loss", combine_loss.item(), global_step)
#                     self.writer.add_scalar("Train/Score", score, global_step)
#                     global_step += 100
...
#             self.writer.add_scalar("Eval/Loss_Var", lossvar_eval, epoch)
#             self.writer.add_scalar("Eval/Loss_Nino", lossnino_eval, epoch)
#             self.writer.add_scalar("Eval/Combine_Loss", comloss_eval, epoch)
#             self.writer.add_scalar("Eval/Score", sceval, epoch)
...
from torch.utils.tensorboard import SummaryWriter  # 導入 TensorBoard
...
class NinoTrainer:
    ...
    def calscore(self, y_pred, y_true, step, phase="Train"):
    	
        # 計算 Nino 分數
        with torch.no_grad():
        	
            print("y_pred shape:", y_pred.shape, "y_true shape:", y_true.shape)
            print("y_pred variance:", torch.var(y_pred, dim=0).mean().item())
            print("y_true variance:", torch.var(y_true, dim=0).mean().item())
            print("y_true min:", y_true.min().item(), "y_true max:", y_true.max().item())
            ...
            # 取 cor 的平均值
            cor_mean = cor.mean()
            print("Correlation (cor):", cor_mean.item(), "acc:", acc.item(), "rmse:", rmse.item())
            # 記錄 acc 和 rmse 到 TensorBoard
            self.writer.add_scalar(f"{phase}/Acc", acc.item(), step)
            self.writer.add_scalar(f"{phase}/RMSE", rmse.item(), step)
            self.writer.add_scalar(f"{phase}/Correlation", cor.mean().item(), step)
        return sc.item()

    def loss_var(self, y_pred, y_true):
        # 場數據的 RMSE
        rmse = torch.mean((y_pred - y_true) ** 2, dim=[3, 4])  # 對 H, W 平均
        # print("loss_var before sqrt:", torch.mean((y_pred - y_true) ** 2, dim=[3, 4]))	
        rmse = rmse.sqrt().mean(dim=0)  # 對時間平均
	# print("loss_var after sqrt:", rmse.sqrt().mean(dim=0))
    ...
    def train_model(self, train_dataset, eval_dataset):
        ...
        best = -math.inf
        sv_ratio = 1
        global_step = 0  # 在方法範圍內初始化 global_step
        ...
        for epoch in range(self.mypara.num_epochs):
            ...
            for j, (input_var, var_true) in enumerate(dataloader_train):
                ...
                loss_var = self.loss_var(var_pred, var_true)
                loss_nino = self.loss_nino(nino_pred, nino_true)
                score = self.calscore(nino_pred, nino_true, step=global_step, phase="Train")
                combine_loss = self.combine_loss(loss_var, loss_nino)
                combine_loss.backward()
                self.opt.step()
                ...
                if j % 100 == 0:
                    ...
                    self.writer.add_scalar("Train/Combine_Loss", combine_loss.item(), global_step)
                    self.writer.add_scalar("Train/Score", score, global_step)
                    global_step += 100  # 每 100 批次記錄一次，步數增加
