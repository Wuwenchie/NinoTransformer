...
def train_data_speration():
    input_dir = './data/all_data'
    input_nc_file = "./data/all_data/GFDL-CM4_sss_sst_1990_2009.nc"  # 輸入檔案路徑
    output_dir = "./data/all_data/test_dataset"  # 輸出資料夾
    input_length = 12
    output_length = 20
    stride = 1

    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, 'GFDL-CM4_test_1990_2009.nc')

    # === 讀取 .nc 檔案 ===
    ds = xr.open_dataset(input_nc_file)
    # print(ds)
    # new_ds = ds.fillna(0.0)
    # new_ds.to_netcdf(os.path.join(output_dir, 'GFDL-CM4_test_1977_2009_1.nc'))

    # ds_1 = xr.open_dataset(os.path.join(output_dir, 'GFDL-CM4_test_1977_2009_1.nc'))
    print("GFDL-CM4_sss_sst_1990_2009 dataset:\n")
    print(ds)
    # 取得原始變數
    temperature = ds['temperatureNor'].isel(n_model=0)  # shape: (time, lev, lat, lon)
    sss = ds['sssNor'].isel(n_model=0)
    sss_ori = ds['sssNor'].values
    temp = ds['temperatureNor'].values


    print("sst_ori_region contains NaN:", np.any(np.isnan(sss_ori)))
    print("temp_ori_region contains NaN:", np.any(np.isnan(temp)))
    # nino34 = ds['nino34']

    # 取得維度大小
    time_len = temperature.shape[0]
    ...
    # 計算可用的樣本數
    num_samples = time_len - input_length - output_length + 1

    # 初始化資料儲存陣列
    temperature_in = np.zeros((num_samples, input_length, len(lev), len(lat), len(lon)))
    temperature_out = np.zeros((num_samples, output_length, len(lev), len(lat), len(lon)))
    sss_in = np.zeros((num_samples, input_length, len(lat), len(lon)))
    sss_out = np.zeros((num_samples, output_length, len(lat), len(lon)))

    # 資料切片
    for i in range(num_samples):
        temperature_in[i] = temperature[i : i+input_length].values
        temperature_out[i] = temperature[i+input_length : i+input_length+output_length].values
        sss_in[i] = sss[i : i+input_length].values
        sss_out[i] = sss[i+input_length : i+input_length+output_length].values
        # nino_out[i] = nino34[i+input_length:i+input_length+output_length].values

    # 組成 xarray.Dataset
    new_ds = xr.Dataset(
        {
            "temperatureNor_in": (["group", "lookback", "lev", "lat", "lon"], temperature_in),
            "temperatureNor_out": (["group", "Tout", "lev", "lat", "lon"], temperature_out),
            "sssNor_in": (["group", "lookback", "lat", "lon"], sss_in),
            "sssNor_out": (["group", "Tout", "lat", "lon"], sss_out),
        },
        coords={
            "group": np.arange(num_samples),
            "lookback": np.arange(input_length),
            "Tout": np.arange(output_length),
            "lev": lev,
            "lat": lat,
            "lon": lon,
        }
    ...
    print(new_ds)
    # 儲存為 netCDF 檔
    new_ds.to_netcdf(output_path, format="NETCDF4", engine='netcdf4')

def test_data_speration(input_nc_file, output_dir, output_filename):
    # output_dir = './data/testing_data'
    # input_nc_file = "./data/testing_data/GFDL-CM4_sss_sst_2010_2024_1.nc"  # 輸入檔案路徑
    # output_dir = "./data/testing_data/test_dataset"  # 輸出資料夾
    input_length = 12
    output_length = 20
    total_length = 32
    stride = 1
    ...
    # 計算可用的樣本數
    # num_samples = time_len - input_length - output_length + 1
    num_samples = (ds.dims['n_mon'] - input_length - output_length) // stride + 1

    # 初始化資料儲存陣列
    temperature_np = np.zeros((num_samples, input_length, len(lev), len(lat), len(lon)))
    sss_np = np.zeros((num_samples, input_length, len(lat), len(lon)))

    # 資料切片
    for i in range(num_samples):
        temperature_np[i] = temperature[i : i+input_length].values
        sss_np[i] = sss[i : i+input_length].values
        # nino_out[i] = nino34[i+input_length:i+input_length+output_length].values

    # 組成 xarray.Dataset
    new_ds = xr.Dataset(
        {
            "temperatureNor": (["group", "n_mon", "lev", "lat", "lon"], temperature_np),
            "sssNor": (["group", "n_mon", "lat", "lon"], sss_np),
            # "stdtemp": (["lev", "lat", "lon"], stdtemp),
            # "stdsss": (["lat", "lon"], stdsss),
        },
        coords={
            "group": np.arange(num_samples),
            "n_mon": np.arange(input_length),
            "lev": lev,
            "lat": lat,
            "lon": lon,
        }
    ...
    stdsss = xr.DataArray(stdsss, 
                            ...
                            name='stdsss')    
    # 合併所有數據
    ds_new = xr.merge([new_ds, stdtemp, stdsss])
    print("test_dataset:\n")
    print(ds_new)
    # 儲存為 netCDF 檔
...
def training_data_pipeline():
    ...
    sss_dir = "./data/training_data/SSS/"
    sst_dir = "./data/training_data/SST/"
    ...
    out_sss = 'sos_1850_2009_conlon.nc'
    out_sst = 'tos_1850_2009_conlon.nc'
    # convert_lon(sss_path, path, out_sss)
    # convert_lon(sst_path, path, out_sst)

    # 2. 補齊缺值
    sss_file = path + 'sos_1850_2009_conlon.nc'
    sst_file = path + 'tos_1850_2009_conlon.nc'
    sss_data = xr.open_dataset(sss_file)
    sst_data = xr.open_dataset(sst_file)
    print(sss_data)
    print(sst_data)
    sss_data_filled = sss_data.fillna(0)
    sst_data_filled = sst_data.fillna(0)

    sss_data_filled.to_netcdf('sos_1850_2009_conlon_1.nc')
    sst_data_filled.to_netcdf('tos_1850_2009_conlon_1.nc')
    ds_sss = xr.open_dataset(path+'sos_1850_2009_conlon_1.nc')
    ds_sst = xr.open_dataset(path+'tos_1850_2009_conlon_1.nc')
    sss_ori = ds_sss['sos'].values
    sst_ori = ds_sst['tos'].values
    print("sss_ori_region contains NaN:", np.any(np.isnan(sss_ori)))
    print("sst_ori_region contains NaN:", np.any(np.isnan(sst_ori)))
    

    # 3. 線性插植
    sss_con = path + 'sos_1850_2009_conlon_1.nc'
    sst_con = path + 'tos_1850_2009_conlon_1.nc'
    interpolated(sss_con, 'sos', sss_inter_dir, 'sss')
    interpolated(sst_con, 'tos', sst_inter_dir, 'sst')

    inter_out_sss = 'sss_inter_1850_2009.nc'
    inter_out_sst = 'sst_inter_1850_2009.nc'
    nc_combine(sss_inter_dir, 'sos', path, inter_out_sss)
    nc_combine(sst_inter_dir, 'tos', path, inter_out_sst)

    # 4. 計算異常值
    sss_interpolated_file = path + "sss_inter_1850_2009.nc"
    sst_interpolated_file = path + "sst_inter_1850_2009.nc"

    anomaly(sss_interpolated_file, 'sos', '1961-01-16', '1990-12-16', path, 'sss_anomaly_1850_2009.nc')
    anomaly(sst_interpolated_file, 'tos', '1961-01-16', '1990-12-16', path, 'sst_anomaly_1850_2009.nc')
    
    # 5. 標準化 (Normalization)   從這裡開始執行
    sss_anomaly_file = path + "sss_anomaly_1850_2009.nc"
    sst_anomaly_file = path + "sst_anomaly_1850_2009.nc"

    expansion(sss_anomaly_file, 'sos')
    expansion(sst_anomaly_file, 'tos')

    sss_convert = './converted_dataset_sss_1850_2009.nc'
    sst_convert = './converted_dataset_sst_1850_2009.nc'
    ds = xr.open_dataset(sst_convert)
    print(ds)
    norm(sss_convert, 'sss', path, 'sss_norm_1850_2014.nc', 'sssNor')
    norm(sst_convert, 'temp', path, 'sst_norm_1850_2014.nc', 'temperatureNor')
    sst_norm = path + 'sss_norm_1850_2009.nc'
    ds = xr.open_dataset(sst_norm)
    print(ds)

    # 6. 合併 SST 和 SSS 數據
    sss_norm_file = path + "sss_norm_1850_2009.nc"
    sst_norm_file = path + "sst_norm_1850_2009.nc"
    merge_with_nino34(sss_norm_file, sst_norm_file, path, 'GFDL-CM4_sss_sst_1850_2009.nc')

    final_file = path + 'GFDL-CM4_sss_sst_1850_2009.nc'
    ds_1 = xr.open_dataset(final_file)
    print(ds_1)

    # final_file_1 = 'GFDL-CM4_sss_sst_1850_1976.nc'
    # final_file_2 = 'GFDL-CM4_sss_sst_1977_2009.nc'
    # sperate_with_time(final_file, '1850-01', '1976-12', path, final_file_1)
    # sperate_with_time(final_file, '1977-01', '2009-12', path, final_file_2)
...
def testing_data_pipeline():
    ...
    sss_dir = "./data/testing_data/SSS/"
    sst_dir = "./data/testing_data/SST/"
    sss_inter_dir = './data/testing_data/sss_inter/'
    sst_inter_dir = './data/testing_data/sst_inter/'
    ...
    # ds = xr.open_dataset(output_filename_sss)
    # print(ds)
    ...
    sst_path = path + "sst_2010_2024.nc"

    out_sss = 'sos_2010_2024_conlon.nc'
    out_sst = 'tos_2010_2024_conlon.nc'
    convert_lon(sss_path, path, out_sss)
    convert_lon(sst_path, path, out_sst)

    # 2. 補齊缺值
    sss_file = path + 'sos_2010_2024_conlon.nc'
    sst_file = path + 'tos_2010_2024_conlon.nc'
    sss_data = xr.open_dataset(sss_file)
    sst_data = xr.open_dataset(sst_file)
    sss_data_filled = sss_data.fillna(0)
    sst_data_filled = sst_data.fillna(0)

    sss_data_filled.to_netcdf('sos_2010_2024_conlon.nc')
    sst_data_filled.to_netcdf('tos_2010_2024_conlon.nc')

    # 3. 線性插植
    sss_con = path + 'sos_2010_2024_conlon.nc'
    sst_con = path + 'tos_2010_2024_conlon.nc'
    interpolated(sss_con, 'sos', sss_inter_dir, 'sss')
    interpolated(sst_con, 'tos', sst_inter_dir, 'sst')

    inter_out_sss = 'sss_inter_2010_2024.nc'
    inter_out_sst = 'sst_inter_2010_2024.nc'
    nc_combine(sss_inter_dir, 'sos', path, inter_out_sss)
    nc_combine(sst_inter_dir, 'tos', path, inter_out_sst)

    # 4. 計算異常值
    sss_interpolated_file = path + "sss_inter_2010_2024.nc"
    sst_interpolated_file = path + "sst_inter_2010_2024.nc"

    anomaly(sss_interpolated_file, 'sos', '2010-01-16', '2024-12-16', path, 'sss_anomaly_1850_2009.nc')
    anomaly(sst_interpolated_file, 'tos', '2010-01-16', '2024-12-16', path, 'sst_anomaly_1850_2009.nc')
    
    # 5. 標準化 (Normalization)   從這裡開始執行
    sss_anomaly_file = path + "sss_anomaly_2010_2024.nc"
    sst_anomaly_file = path + "sst_anomaly_2010_2024.nc"

    expansion(sss_anomaly_file, 'sos')
    expansion(sst_anomaly_file, 'tos')

    sss_convert = './converted_dataset_sss_2010_2024.nc'
    sst_convert = './converted_dataset_sst_2010_2024.nc'
    ds = xr.open_dataset(sst_convert)
    print(ds)
    norm(sss_convert, 'sss', path, 'sss_norm_2010_2024.nc', 'sssNor')
    norm(sst_convert, 'temp', path, 'sst_norm_2010_2024.nc', 'temperatureNor')
    sst_norm = path + 'sss_norm_2010_2024.nc'
    ds = xr.open_dataset(sst_norm)
    print(ds)

    # 6. 合併 SST 和 SSS 數據
    sss_norm_file = path + "sss_norm_2010_2024.nc"
    sst_norm_file = path + "sst_norm_2010_2024.nc"
    merge_with_nino34(sss_norm_file, sst_norm_file, path, 'GFDL-CM4_sss_sst_nino_2010_2024.nc')
    # merge(sss_norm_file, sst_norm_file, path, 'GFDL-CM4_sss_sst_2010_2024.nc')

    final_file = path + 'GFDL-CM4_sss_sst_nino_2010_2024.nc'
    ds_1 = xr.open_dataset(final_file)
    print(ds_1)
...
def all_data():
    ...
    sst_dir = "./data/all_data/SST/"
    sss_inter_dir = './data/all_data/sss_inter/'
    sst_inter_dir = './data/all_data/sst_inter/'

    # nc_combine(sss_dir, 'sos', output_dir, 'sos_1850_2024.nc')
    # nc_combine(sst_dir, 'tos', output_dir, 'tos_1850_2024.nc')

    # 1. 經度轉換(平移)
    sss_path = output_dir + 'sos_1850_2024.nc'
    sst_path = output_dir + 'tos_1850_2024.nc'

    out_sss = 'sos_1850_2024_conlon.nc'
    out_sst = 'tos_1850_2024_conlon.nc'
    # convert_lon(sss_path, output_dir, out_sss)
    # convert_lon(sst_path, output_dir, out_sst)

    # 2. 補齊缺值
    sss_file = output_dir + 'sos_1850_2024_conlon.nc'
    sst_file = output_dir + 'tos_1850_2024_conlon.nc'
    sss_data = xr.open_dataset(sss_file)
    sst_data = xr.open_dataset(sst_file)
    print(sss_data)
    print(sst_data)
    sss_data_filled = sss_data.fillna(0.0)
    sst_data_filled = sst_data.fillna(0.0)

    # sss_data_filled.to_netcdf(os.path.join(output_dir, 'sos_1850_2024_conlon_1.nc'))
    # sst_data_filled.to_netcdf(os.path.join(output_dir, 'tos_1850_2024_conlon_1.nc'))
    ds_sss = xr.open_dataset(output_dir+'sos_1850_2024_conlon_1.nc')
    ds_sst = xr.open_dataset(output_dir+'tos_1850_2024_conlon_1.nc')
    sss_ori = ds_sss['sos'].values
    sst_ori = ds_sst['tos'].values
    print("sss_ori_region contains NaN:", np.any(np.isnan(sss_ori)))
    print("sst_ori_region contains NaN:", np.any(np.isnan(sst_ori)))
    ...
    sst_con = output_dir + 'tos_1850_2024_conlon_1.nc'
    # interpolated(sss_con, 'sos', sss_inter_dir, 'sss')
    # interpolated(sst_con, 'tos', sst_inter_dir, 'sst')

    inter_out_sss = 'sss_inter_1850_2024.nc'
    inter_out_sst = 'sst_inter_1850_2024.nc'
    # nc_combine(sss_inter_dir, 'sos', output_dir, inter_out_sss)
    # nc_combine(sst_inter_dir, 'tos', output_dir, inter_out_sst)

    # 4. 計算異常值
    sss_interpolated_file = output_dir + "sss_inter_1850_2024.nc"
    sst_interpolated_file = output_dir + "sst_inter_1850_2024.nc"

    anomaly(sss_interpolated_file, 'sos', '1961-01-16', '1990-12-16', output_dir, 'sss_anomaly_1850_2024.nc')
    anomaly(sst_interpolated_file, 'tos', '1961-01-16', '1990-12-16', output_dir, 'sst_anomaly_1850_2024.nc')
    
    # 5. 標準化 (Normalization)   從這裡開始執行
    sss_anomaly_file = output_dir + "sss_anomaly_1850_2024.nc"
    sst_anomaly_file = output_dir + "sst_anomaly_1850_2024.nc"

    expansion(sss_anomaly_file, 'sos')
    expansion(sst_anomaly_file, 'tos')

    sss_convert = './converted_dataset_sss_1850_2024.nc'
    sst_convert = './converted_dataset_sst_1850_2024.nc'
    ds = xr.open_dataset(sst_convert)
    print(ds)
    norm(sss_convert, 'sss', output_dir, 'sss_norm_1850_2024.nc', 'sssNor')
    norm(sst_convert, 'temp', output_dir, 'sst_norm_1850_2024.nc', 'temperatureNor')
    sst_norm = output_dir + 'sss_norm_1850_2024.nc'
    ds = xr.open_dataset(sst_norm)
    print(ds)

    # 6. 合併 SST 和 SSS 數據
    sss_norm_file = output_dir + "sss_norm_1850_2024.nc"
    sst_norm_file = output_dir + "sst_norm_1850_2024.nc"
