...
for i_file in files[:file_num + 1]:
    ...
    # 繪製技能等高線圖
    ax2.contourf(nino_skill, levels=np.arange(0, 1.01, 0.1), extend="both", cmap="RdBu_r")
    ...
    y_ticklabel = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    ax2.set_yticklabels(y_ticklabel, fontsize=9)
    ax2.set_ylabel("Month", fontsize=9)
    del tem1, tem2

    # 添加標籤並保存圖片
    ax1.text(x=0.02, y=1.02, s="(a)", fontsize=9)
    ax2.text(x=0.02, y=11.24, s="(b)", fontsize=9)
    plt.tight_layout()
